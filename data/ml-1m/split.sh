...
RATINGS_COUNT=`wc -l ratings_new.dat | xargs | cut -d ' ' -f 1`
...
for i in 1 2 3 4 5
  do
    head -`expr $i \* $SET_SIZE` ratings_new.dat | tail -$SET_SIZE > r$i.test
    ...
    head -`expr \( $i - 1 \) \* $SET_SIZE` ratings_new.dat > r$i.train
    tail -`expr \( 5 - $i \) \* $SET_SIZE` ratings_new.dat >> r$i.train

    if [ $i -eq 5 ]; then
       tail -$REMAINDER ratings_new.dat >> r5.test
    else
       tail -$REMAINDER ratings_new.dat >> r$i.train
    fi
